...
@implementation SLFAbstractCodableObject
...
{
    if (!SLFTypeNonEmptyStringOrNil(key))
        return nil;
...
- (NSUInteger)hash
{
    ...
    [[[self class] codableKeysAndClasses] enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        current = [SLFAbstractCodableObject valueHashForHash:[[strongSelf valueForKey:key] hash] index:hashIndex] ^ current;
        hashIndex++;
