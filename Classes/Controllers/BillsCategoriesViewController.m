...
- (IBAction)loadCategoriesForChamber:(SLFChamber *)newChamber session:(NSString *)session;
- (void)loadCategories:(id)sender;
@end
...
- (void)viewDidLoad {
	...
	[[NSNotificationCenter defaultCenter] addObserver:self
											 selector:@selector(loadCategories:) 
...
- (NSString *)chamber {
	NSInteger theChamber = CHAMBER_ALL;
	if (chamberControl)
...
- (void)loadCategories:(id)sender {
    ...
    for (SLFChamber *aChamber in state.chambers)
        [self loadCategoriesForChamber:aChamber session:session];
}
...
- (IBAction)loadCategoriesForChamber:(SLFChamber *)newChamber session:(NSString *)session {
    ...
    NSMutableString *resourcePath = [NSMutableString stringWithFormat:@"/subject_counts/%@/%@/", newChamber.stateID, session];
    if (![newChamber.type isEqualToString:@"all"])
        [resourcePath appendFormat:@"%@/",  newChamber.type];
...
- (void)createChamberControl {	
    ...
    SLFState *state = [[StateMetaLoader sharedStateMeta] selectedState];
    for (SLFChamber *aChamber in state.chambers) {
        [segmentItems addObject:aChamber.shortName];
    }
	UISegmentedControl *ctl = [[UISegmentedControl alloc] initWithItems:segmentItems];
...
- (void)request:(RKRequest*)request didLoadResponse:(RKResponse*)response {  
	if ([request isGET] && [response isOK]) {  
		...
		[newArray sortUsingDescriptors:[NSArray arrayWithObject:desc]];
            ...
            // TODO: need to use RKPathMatcher here instead of assuming selected/singleton settings
        StateMetaLoader *meta = [StateMetaLoader sharedStateMeta];
        if (!meta.selectedState || IsEmpty(meta.selectedSession))
            return;
        SLFState *state = meta.selectedState;
        ...
        NSInteger chamberTypeValue = loadedChamber.typeValue;
		[categories_ setObject:newArray forKey:[NSString stringWithFormat:@"%d", chamberTypeValue]];
		[newArray release];
