...
- (void)setupHeader {	
	...
	NSString *billTitle = [NSString stringWithFormat:@"(%@) %@", session, bill.billID];
	self.navigationItem.title = billTitle;
	self.actionHeader.titleLabel.text = billTitle;
	[self.actionHeader setNeedsDisplay];
	
	NSDictionary *currentAction = [bill.actions lastObject];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	...
	if (bill) {
		switch (section) {
			case kBillSubjects:
				rows = [bill.subjects count];
				break;
			case kBillSponsors:
				rows = [bill.sponsors count];
				break;
			case kBillVersions:
				rows = [bill.versions count];
				break;
			case kBillActions:
				rows = [bill.actions count];
				break;
			case kBillVotes:
				rows = [bill.votes count];
				break;
			default:
				break;
		}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (bill) {
		switch ([indexPath section]) {
			...
			case kBillSubjects:
			{
				NSString *subject = [bill.subjects objectAtIndex:indexPath.row];
			...
			case kBillSponsors: 
			{
				NSDictionary *sponsor = [bill.sponsors objectAtIndex:indexPath.row];
			...
			case kBillVersions: 
			{				
				NSDictionary *version = [bill.versions objectAtIndex:indexPath.row];
                
				NSString *senateString = bill.state.upperChamberName;
				NSString *houseString = bill.state.lowerChamberName;
				...
				if ([@"tx" isEqual:bill.stateID]) { // Texas sometimes screws this up.
					if ([name hasSuffix:@"I"])
			...
			case kBillVotes: 
			{				
				NSDictionary *vote = [bill.votes objectAtIndex:indexPath.row];
                ...
                SLFChamber *chamber = [SLFChamber chamberWithType:[vote objectForKey:@"chamber"] forState:bill.state];
				NSString * chamberString = chamber.shortName;
			...
			case kBillActions: 
			{
				NSDictionary *currentAction = [bill.actions objectAtIndex:indexPath.row];
				...
				if (!IsEmpty([currentAction objectForKey:@"action"])) {
					...
					if (!IsEmpty([currentAction objectForKey:@"actor"])) {
                        ...
                        SLFChamber *chamber = [SLFChamber chamberWithType:[currentAction objectForKey:@"actor"] forState:bill.state];
                        desc = [NSString stringWithFormat:@"(%@) %@", chamber.name, [currentAction objectForKey:@"action"]];
					}
...
- (void)tableView:(UITableView *)aTableView didSelectRowAtIndexPath:(NSIndexPath *)newIndexPath {
	...
	switch (newIndexPath.section) {
		case kBillSubjects: {
			NSString *subject = [bill.subjects objectAtIndex:newIndexPath.row];
			...
			break;
		case kBillSponsors: {
			NSDictionary *sponsor = [bill.sponsors objectAtIndex:newIndexPath.row];
			[self showLegislatorDetailsWithOpenStatesID:[sponsor objectForKey:@"leg_id"]];
		}
			break;
		case kBillVersions: {
			NSDictionary *version = [bill.versions objectAtIndex:newIndexPath.row];
			if (version) {
			...
			break;
		case kBillVotes: {
			NSMutableDictionary *vote = [bill.votes objectAtIndex:newIndexPath.row];
			if (vote) {
				...
				NSString *titleString = [NSString stringWithFormat:NSLocalizedStringFromTable(@"%@ %@ Vote", @"DataTableUI", @"As in HB 323 Final Passage Vote"), 
										 bill.billID, [[vote objectForKey:@"motion"] firstLetterCapitalized]];
...
- (void)request:(RKRequest*)request didLoadResponse:(RKResponse*)response {  
	if ([request isGET] && [response isOK]) {  
		...
		NSDictionary *tagBill = [NSDictionary dictionaryWithObject:bill.watchID forKey:@"bill"];
