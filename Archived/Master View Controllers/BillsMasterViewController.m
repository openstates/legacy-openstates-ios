...
- (void)viewDidLoad {
    ...
    for (SLFChamber *chamber in state.chambers) {
        [buttonTitles addObject:chamber.shortName];
    }
    self.searchDisplayController.searchBar.scopeButtonTitles = buttonTitles;
...
//	IF WE'RE CLICKING ON SOME SEARCH RESULTS ... PULL UP THE BILL DETAIL VIEW CONTROLLER
	if (aTableView == self.searchDisplayController.searchResultsTableView) { // we've clicked in a search table
		...
		if ([dataObject isKindOfClass:[SLFBill class]]) {
			...
			if ([self.detailViewController respondsToSelector:@selector(setDataObject:)])
				[self.detailViewController performSelector:@selector(setDataObject:) withObject:bill];
