...
- (NSMutableDictionary *)standardizeEvent:(NSDictionary *)inEvent forState:(SLFState *)state;
@end
...
- (NSString *)pathToEventCacheWithRequest:(RKRequest *)request {
	SLFState *loadedState = eventState; // default to what we stored at the time of our last request
	
	if (request && request.userData && [request.userData isKindOfClass:[SLFState class]]) {
	...
	NSString *stateEventsCache = [NSString stringWithFormat:@"%@-%@", loadedState.stateID, kCalendarEventsCacheFile];
...
- (void)request:(RKRequest*)request didLoadResponse:(RKResponse*)response {  
	...
	if ([request isGET] && [response isOK]) {  
        ...
        SLFState *state = request.userData;

		_events = [[NSMutableArray alloc] init];
		for (NSDictionary *event in allEvents) {
			NSMutableDictionary *newEvent = [self standardizeEvent:event forState:state];	// clean up events, parsing for our needs
			if (newEvent) {
...
- (NSMutableDictionary *)standardizeEvent:(NSDictionary *)inEvent forState:(SLFState *)state {
	...
	@try {
		...
		if (!IsEmpty(participants)) {
			...
			if (participant) {
                ...
                if (!IsEmpty(chamberString)) {
                    SLFChamber *chamber = [SLFChamber chamberWithType:chamberString forState:state];
					[loadedEvent setObject:chamber forKey:kCalendarEventsTypeChamberValue];
                }
			}
		}
		...
		SLFChamber *chamberValue = [loadedEvent objectForKey:kCalendarEventsTypeChamberValue];
		if (chamberValue) {
			chamberName = chamberValue.shortName;		
		}
