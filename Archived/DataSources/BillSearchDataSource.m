...
- (NSIndexPath *)indexPathForDataObject:(id)dataObject {
    SLFBill *bill = (SLFBill *)dataObject;
	if (dataObject && [dataObject isKindOfClass:[SLFBill class]] && bill.billID) {

#if USE_BILLTYPES_AS_SECTIONS
		NSString *typeString = billTypeStringFromBillID(bill.billID);
...
- (void) generateSections {
    ...
    {				
		NSString *c = billTypeStringFromBillID(bill.billID);
    ...
    {
		[[_sections objectForKey:key] sortUsingComparator:^(SLFBill *item1, SLFBill *item2) {
			NSString *bill_id1 = item1.billID;
			NSString *bill_id2 = item2.billID;
...
- (void)configureCell:(TexLegeStandardGroupCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
	...
	SLFBill *bill = [self dataObjectForIndexPath:indexPath];
	if (!bill || [[NSNull null] isEqual:bill])
		return;  // ?????
	
	NSString *bill_id = bill.billID;
...
- (void)pruneBillsForAuthor:(NSString *)sponsorID {
	...
	for (SLFBill *bill in tempRows) {
		BOOL found = NO;
		BOOL hasSponsors = NO;
		
		for (NSDictionary *sponsor in bill.sponsors) {
			hasSponsors = YES;
...
#warning This is currently not set up correctly for SLFBill
...
- (void)request:(RKRequest*)request didLoadResponse:(RKResponse*)response {  
	...
	if ([request isGET] && [response isOK]) {  
		...
		else if ([results isKindOfClass:[SLFBill class]])
			[_rows addObject:results];

		// if we wanted blocks, we'd do this instead:
		[_rows sortUsingComparator:^(SLFBill *item1, SLFBill *item2) {
			NSString *bill_id1 = item1.billID;
			NSString *bill_id2 = item2.billID;
			return [bill_id1 compare:bill_id2 options:NSNumericSearch];
		}];
